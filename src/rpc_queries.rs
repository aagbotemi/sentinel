...
use alloy::{primitives::{Address, TxHash, U256}, providers::{Provider, ProviderBuilder}, rpc::types::eth::{Block, BlockId, Transaction}};
use std::error::Error;
...
pub async fn get_native_balance(
    ...
    let provider = ProviderBuilder::new().on_http(rpc_url.parse()?)?;
    let balance = provider.get_balance(user_address, BlockId::latest()).await?;
